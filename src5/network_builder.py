...
def _perturbed(base_km, rng):
    # each direction gets a slightly different weight, to model one-way
    # systems / gradients / congestion instead of a perfectly symmetric road
...
# Small hand-built graphs to demonstrate Bellman-Ford's negative-weight
# and negative-cycle handling (real road distances are never negative,
# so these model discounted/subsidised routes instead).
